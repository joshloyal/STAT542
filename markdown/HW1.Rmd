...
read_chunk('../R/model_selection.R')
```
...
# euclidean distance
preds <- predict_knn(X = splits$X_train, Y = splits$y_train, X_query = splits$X_test, 
                     k = 5)
print(mean_squared_error(preds, splits$y_test))

# Mahalanobis distance
mydist3 <- function(x, y, s) {
  mahalanobis(x, center = y, cov = s)
}
sigma = sample_covariance(splits$X_train)
preds <- predict_knn(X = splits$X_train, Y = splits$y_train, X_query = splits$X_test, 
                     k = 5, dist_func = mydist3, s = sigma)
print(mean_squared_error(preds, splits$y_test))
```
